...
function determinePolyArea(v, rangedThreshold, rangedArea, threshold, k) {
    var best = 1.0;
    var bestD = 0;
...
function getEarthWork(data, slope, superWidth, superDepth, threshold, rangedThreshold) {
    ...
    for (var i = triangles.length; i >= 3; i-=3) {
        ...
        //determine range
        var t1 = determinePolyArea(v1, rangedThreshold, rangedArea, threshold, i / 3 - 1);
        var t2 = determinePolyArea(v2, rangedThreshold, rangedArea, threshold, i / 3 - 1);
        var t3 = determinePolyArea(v3, rangedThreshold, rangedArea, threshold, i / 3 - 1);
    ...
    sum += getSideWork(data, triangles, slope, superWidth, superDepth, threshold, rangedThreshold, rangedArea);
    sum = Math.round(sum);
    return sum;
}

function getSideWork(vertex, triangles, slope, superWidth, superDepth, threshold, rangedThreshold, rangedArea) {
    var borders = getBorder(vertex, triangles);
    var sum = 0.0;
    for (var i = 0; i < borders.length; i++) {
        var t1 = determinePolyArea({x:vertex[borders[i].v1][1], y:vertex[borders[i].v1][0]}, rangedThreshold, rangedArea, threshold);
        var t2 = determinePolyArea({x:vertex[borders[i].v2][1], y:vertex[borders[i].v2][0]}, rangedThreshold, rangedArea, threshold);
...
function getBorder(vertex, triangles) {
    ...
    for (var i = triangles.length; i >= 3; i-=3) {
        ...
        var a = testdistance(vertex[v1][0], vertex[v1][1], vertex[v2][0], vertex[v2][1]);
        var b = testdistance(vertex[v1][0], vertex[v1][1], vertex[v3][0], vertex[v3][1]);
        var c = testdistance(vertex[v2][0], vertex[v2][1], vertex[v3][0], vertex[v3][1]);
        if (a <= minimunInterval && b <= minimunInterval && c <= minimunInterval) {
            checkEdge(edgeSet[v1], borders[v1], v2);
            checkEdge(edgeSet[v2], borders[v2], v1);
            checkEdge(edgeSet[v1], borders[v1], v3);
            checkEdge(edgeSet[v3], borders[v3], v1);
            checkEdge(edgeSet[v2], borders[v2], v3);
            checkEdge(edgeSet[v3], borders[v3], v2);
        } 
    }
    var result = [];
    for (var i = 0; i < edgeSet.length; i++) {
