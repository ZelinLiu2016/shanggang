...
function Test_AddShip() {    
    ...
    if (iCurShipCount > 0) {
        ...
        API_SetMapViewCenter(123.5, 31.5, 640000);
        return;
    }
    ...
    API_SetMapViewCenter(117.7014, 38.9856, 640000); //切换到有船舶的区域,调用了这个接口，可以不再调用API_ReDrawShips
...
function PlayShipHistoryButton(strType) {
    ...
    if (strType == "start") {//开始
        ...
        API_SetMapViewCenter(121.5, 34.5, 640000); //切换到有港口的区域，这里切换过去的时候，会刷新海图，所以不用调用API_ReDrawLayer();
