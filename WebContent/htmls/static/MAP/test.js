...
function Test_AddShip() {    
    ...
    //API_SetShipShowNameFrameStyle(10000, 200, 30, 2, "#FF0000", 100);
    //添加船舶
    var strShipInfos = "";
    //添加或者更新100艘船舶

    var iAddShipCount = 100;
    for (var i = 0; i < iAddShipCount; i++) {
        var shipId = i;
        var shipMMSI = i; //mmsi
        var shipName = ""; //船名
        var shipGeoPoX = parseInt(1211000000) + parseInt(Math.random() * 10000000); //经度
        var shipGeoPoY = parseInt(313000000) + parseInt(Math.random() * 10000000); //纬度

        //var shipGeoPoX = parseInt(1177014000); //经度
        //var shipGeoPoY = parseInt(389856000); //纬度
    ...
    //API_SetMapViewCenter(117.7014, 38.9856, null); //切换到有船舶的区域,调用了这个接口，可以不再调用API_ReDrawShips
...
function AddSomeWeatherInfo() {
    ...
    if (iWeatherLayerPos > -1) {
        ...
        API_SetMapViewCenter(123.5, 33.5, null);//切换到有气象的区域
    }
}
...
function AddTyphoon() {
    ...
    API_SetMapViewCenter(126, 29, null); //切换到台风，这里切换过去的时候，会刷新海图，所以不用调用API_ReDrawLayer();
...
function AddSomePort() { 
    ...
    if (iPortLayerPos > -1) {
        ...
        API_SetMapViewCenter(122.2333, 31.4167, null); //切换到有港口的区域，这里切换过去的时候，会刷新海图，所以不用调用API_ReDrawLayer();
    }
}
...
function AddSomePlayHistoryTracksShipInfo(shipCount, bShowTrack) {
    for (var i = 0; i < shipCount; i++) {
        ...
        var shipGeoPoX = parseInt(1210000000) + parseInt(Math.random() * 10000000); //经度
        var shipGeoPoY = parseInt(340000000) + parseInt(Math.random() *  10000000); //纬度
        var shipCourse = parseInt(Math.random() * 360);    //航向
...
function PlayShipHistoryTracks(strType) {
    ...
    if (strType == "ship") {
        //船舶轨迹回放
        AddSomePlayHistoryTracksShipInfo(5,true); //添加50艘测试数据
    }
    else {
        //根据区域回放轨迹
...
function PlayShipHistoryButton(strType) {
    ...
    if (strType == "start") {//开始
        ...
        API_SetMapViewCenter(121.5, 34.5, null); //切换到有港口的区域，这里切换过去的时候，会刷新海图，所以不用调用API_ReDrawLayer();
...
function AddOceanCirculation() {
    if (bAddOceanCirculation == true) {
        ...
        API_SetMapViewCenter(141, 10, null); //切换到有洋流的区域
        return;
    }
...
//测试修改绘制的坐标
function TestSetCurDrawDynamicObjGeoPo() {
    var arrCurObjGeoPo = API_GetCurDrawDynamicObjGeoPo();
    if (arrCurObjGeoPo) {
        var iCount = arrCurObjGeoPo.length;
        var arrNewObjGeoPo = [];
        for (var i = 0; i < iCount; i++) {
            arrNewObjGeoPo.push({ x: parseInt(arrCurObjGeoPo[i].x) + 1000000, y: parseInt(arrCurObjGeoPo[i].y) + 1000000 });
        }
        API_SetCurDrawDynamicObjGeoPo(arrNewObjGeoPo);
        API_SetMapViewCenter(parseFloat(arrCurObjGeoPo[0].x) / 10000000, parseFloat(arrCurObjGeoPo[0].y) / 10000000, null);
    }
}
