...
function Test_AddShip() {    
    ...
    if (iCurShipCount > 0) {
        ...
        API_SetMapViewCenter(121.4133, 31.2238, 160000);
        return;
    }
    ...
    for (var i = 0; i < iAddShipCount; i++) {
        ...
        var shipGeoPoX = parseInt(1214133000); //经度
        var shipGeoPoY = parseInt(312238000); //纬度
    ...
    API_SetMapViewCenter(121.4133, 31.2238, 160000); //切换到有船舶的区域,调用了这个接口，可以不再调用API_ReDrawShips
    var timerId = setInterval(Test_UpdateShipInfo,1000);//3秒更新一次船舶信息
}
...
function AddCurDrawObject() {
    ...
    var arrObjPo = new Array("Saab","Volvo","BMW");
...
function AddSomePlayHistoryTracksShipInfo(shipCount, bShowTrack) {
    for (var i = 0; i < shipCount; i++) {
        ...
        for (var iTrackPos = 0; iTrackPos < 100; iTrackPos++) {
            ...
            if (shipGeoPoX < 1211000000 || shipGeoPoX > 1223000000 || shipGeoPoY < 311200000 || shipGeoPoY > 313600000) {
                shipCourse = parseInt(Math.random() * 100);
            }
...
function PlayShipHistoryTracks(strType) {
    ...
    if (strType == "ship") {
        //船舶轨迹回放
        AddSomePlayHistoryTracksShipInfo(1,true); //添加 艘测试数据
		API_SetMapViewCenter(122.2333, 31.4167, 160000);
    }
    else {
        //根据区域回放轨迹
...
function PlayShipHistoryButton(strType) {
    ...
    if (strType == "start") {//开始
		...
		//console.log(API_GetPlayHistroyTrackInfoByPos(API_GetPlayShipPosById(0), 0));
		//console.log(API_GetPlayShipInfoByPos(API_GetPlayShipPosById(0)));
		var mousemoveobjinfo;
		ReturnOnMouseRightDown(mousemoveobjinfo);
		console.log(mousemoveobjinfo);
